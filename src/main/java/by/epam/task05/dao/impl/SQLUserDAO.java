...
public class SQLUserDAO extends SqlDao implements UserDAO {
	...
	private static final String ROLE_CHECK_CREDENTIONALS = "SELECT * FROM user WHERE `id_role`=?";
	private static final String ALL_USERS_CREDENTIONALS = "SELECT * FROM user";
	...
	private static final String SET_ROLE_CREDENTIONALS = "UPDATE user SET `id_role` = ? WHERE `id_user` = ?";
	private static final String SET_PASSWORD_CREDENTIONALS = "UPDATE user SET `password` = ? WHERE (`id_user` = ? AND `password` = ?)";
	...
	public List<User> getUsers(UserRole role) throws DaoException {
		...
		try {
			con = DriverManager.getConnection(url, login, password);
			st = con.prepareStatement(ROLE_CHECK_CREDENTIONALS);
			st.setInt(1, role.toInt());
			rs = st.executeQuery();
	...
	public List<User> getUsers() throws DaoException {
		...
		try {
			con = DriverManager.getConnection(url, login, password);
			st = con.prepareStatement(ALL_USERS_CREDENTIONALS);
			rs = st.executeQuery();
	...
	@Override
	public boolean setRole(int idUser, UserRole role) throws DaoException
	{
		Connection con = null;
		PreparedStatement st= null;
		ResultSet rs = null;
		...
		try {
			con = ConnectionPool.getInstance().getConnection();
			st = con.prepareStatement(SET_ROLE_CREDENTIONALS);
	...
	@Override
	public boolean setPassword(int idUser, String oldPassword, String newPassword) throws DaoException
	{
		Connection con = null;
		PreparedStatement st= null;
		ResultSet rs = null;
		...
		try {
			con = ConnectionPool.getInstance().getConnection();
			st = con.prepareStatement(SET_PASSWORD_CREDENTIONALS);
